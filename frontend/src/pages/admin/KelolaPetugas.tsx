...
import { Search, ShieldAlert, ShieldCheck, UserPlus } from "lucide-react";
...
const initialStaff: UserItem[] = [
  { id: "101", name: "Siti Rahmawati", identifier: "petugas1@ilkom.com", role: "PETUGAS", status: "Aktif" },
  { id: "102", name: "Rian Hidayat", identifier: "petugas2@ilkom.com", role: "PETUGAS", status: "Aktif" },
];
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div className="flex flex-col gap-1">
          ...
          <p className="text-sm md:text-base text-slate-600 font-medium">
            Manajemen akun staf dan petugas operasional perpustakaan.
          </p>
        </div>

        <Button className="bg-orange-500 hover:bg-orange-600 text-white font-bold border-2 border-blue-900 [box-shadow:3px_3px_0px_#1E3A8A] active:translate-x-[2px] active:translate-y-[2px] active:[box-shadow:0px_0px_0px_#1E3A8A] rounded-md transition-all flex items-center gap-2">
      ...
      <div className="bg-white rounded-lg border-2 border-blue-900 [box-shadow:4px_4px_0px_#1E3A8A] overflow-hidden">
        <div className="p-4 sm:p-6 border-b-2 border-blue-900 flex flex-col sm:flex-row gap-4 items-stretch sm:items-center justify-between bg-slate-50">
          <div className="relative w-full max-w-md">
            ...
            <Input
              type="text"
              placeholder="Cari berdasarkan Email atau Nama Petugas..."
              value={search}
        ...
        <DataTable
          ...
          emptyText="Tidak ada data petugas yang ditemukan."
        />
        ...
        <div className="p-4 border-t-2 border-blue-900 flex items-center justify-between text-xs sm:text-sm font-semibold text-slate-700 bg-slate-50">
          <div>Menampilkan {filteredStaff.length} dari {initialStaff.length} petugas</div>
        </div>
      </div>
        ...
        <AlertDialogContent className="border-2 border-blue-900 [box-shadow:6px_6px_0px_#1E3A8A] rounded-lg">
          <AlertDialogHeader>
            ...
            <AlertDialogDescription className="text-slate-700 pt-2 font-medium">
              Apakah Anda yakin ingin mereset sandi untuk petugas <strong>{selectedUser?.name}</strong> ({selectedUser?.identifier})?
              <br /><br />
