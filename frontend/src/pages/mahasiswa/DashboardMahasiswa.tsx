...
export default function DashboardMahasiswa() {
  ...
  //Menghitung jumlah peminjaman aktif berdasarkan kategori arsip
  let countSkripsi = 0;
  ...
  const transaksiBerjalan = activeBorrowings.find(
    (b) =>
      b.status === "WAITING_PICKUP" ||
      b.status === "BORROWED" ||
      b.status === "REQUESTED",
  );
  ...
  if (transaksiBerjalan && transaksiBerjalan.returnDate) {
    ...
    tenggatTerdekat = returnDate.toLocaleDateString("id-ID", {
      month: "short",
      day: "numeric",
    });
            ...
            <div className="bg-slate-50 sm:bg-white sm:border border-slate-200 sm:shadow-sm rounded-xl sm:rounded-xl flex flex-col sm:flex-row overflow-hidden">
              ...
              <div className="p-5 sm:p-6 flex-1 border-b sm:border-b-0 sm:border-r border-slate-200/60 sm:border-dashed">
                <div className="flex justify-between items-start mb-3">
                  <Badge
                    variant="outline"
                    className={`font-bold px-2.5 py-0.5 text-[10px] sm:text-xs bg-white border-blue-200 text-blue-700`}
                  >
                    {transaksiBerjalan.status.replace("_", " ")}
                  </Badge>
                  <span className="text-[10px] sm:text-xs font-mono text-slate-400 font-bold">
                    #{transaksiBerjalan.id.substring(0, 8).toUpperCase()}
                  </span>
                </div>
                ...
                <p className="text-xs sm:text-sm text-slate-500 mt-1 font-medium">
                  {transaksiBerjalan.archive.author}
                </p>
