...
export function usePetugasProfile() {
  ...
  // 4. PASSWORD STRENGTH METER & VALIDATION
  ...
  const handleUpdateProfile = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.success("Nama profil petugas berhasil diperbarui!", {
        duration: 3000,
      });
      ...
      const msg =
        ...
        err.message ||
        "Gagal memperbarui profil petugas";
  ...
  const handleChangePassword = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.success(
        "Kata sandi berhasil diubah! Sesi akun petugas Anda telah diamankan.",
        { duration: 4500 }
      );
      setCurrentPassword("");
      setNewPassword("");
  ...
  const initials = (user?.name || name || "Petugas")
    .trim()
    .split(" ")
    .filter(Boolean)
    .map((n: string) => n[0])
